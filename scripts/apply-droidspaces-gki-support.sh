...
write_droidspaces_fragment() {
  python3 - "$DROIDSPACES_FRAGMENT" "$COMMON_DIR" <<'PY'
import re
import sys
from pathlib import Path
...
def defined_kconfig_symbols(root: Path) -> tuple[set[str], bool]:
    pattern = re.compile(r"^\s*(?:menu)?config\s+([A-Za-z0-9_]+)\b")
    symbols: set[str] = set()
    saw_kconfig = False
    for kconfig in root.rglob("Kconfig*"):
        if any(part == ".git" for part in kconfig.parts):
            continue
    ...
    return symbols, saw_kconfig

defined_symbols, saw_kconfig = defined_kconfig_symbols(common_dir)
enabled = requested if not saw_kconfig else [option for option in requested if option in defined_symbols]
skipped = [] if not saw_kconfig else [option for option in requested if option not in defined_symbols]

lines = [
    "# Generated by scripts/apply-droidspaces-gki-support.sh",
    "# Droidspaces GKI support fragment.",
    "",
]
lines.extend(f"{option}=y" for option in enabled)
if skipped:
    lines.extend(["", "# Skipped unsupported Droidspaces config symbols:"])
    lines.extend(f"# {option} is not defined in this kernel Kconfig" for option in skipped)
text = "\n".join(lines) + "\n"

previous = path.read_text(encoding="utf-8") if path.exists() else ""
path.write_text(text, encoding="utf-8")
status = "already up to date" if previous == text else "updated"
if skipped:
    status += f"; skipped unsupported: {', '.join(skipped)}"
print(status)
PY
}
...
verify_droidspaces_fragment() {
  python3 - "$DROIDSPACES_FRAGMENT" "$COMMON_DIR" \
    CONFIG_SYSVIPC \
    CONFIG_POSIX_MQUEUE \
    CONFIG_IPC_NS \
    CONFIG_PID_NS \
...
defined_symbols, saw_kconfig = defined_kconfig_symbols(common_dir)
required = requested if not saw_kconfig else [option for option in requested if option in defined_symbols]
unsupported = [] if not saw_kconfig else [option for option in requested if option not in defined_symbols]
...
for option in unsupported:
    ...
    matches = [line for line in lines if pattern.match(line)]
    if matches:
        raise SystemExit(f"{path}: unsupported {option} must not be emitted for this kernel")
print("ok")
PY
}
...
log "Config: enabling Droidspaces fragment"
